...
<table class="box">
    ...
    if($_SERVER['REQUEST_METHOD']=="POST"){
      $readersearch=$_POST['readnamesearch'];
      $sql="SELECT * FROM reader WHERE readername LIKE '%$readersearch%' ";
      $result=mysqli_query($conn,$sql);
      $num=mysqli_num_rows($result);

      if($num>0){
        echo '<div class="alert alert-success alert-dismissible fade show" role="alert">
        <strong>'.$num.' </strong> Readers found!
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>';
        
        
          while($row=mysqli_fetch_assoc($result)){
              echo '<tr><td>'.$row['readerid'].'</td><td>'.$row['readername'].'</td><td>'.$row['readeremail'].'</td><td>'.$row['readerphone'].'</td><td><form action="remread.php" method="POST"><input type="hidden" value="'.$row['readerid'].'" name="currreadid" id="currreadid"><button type="submit" class="btn btn-danger badge-pill">Remove</button></form></td></tr>';
          }
          echo "</table>";
      }
      else if($num==0){
        echo '<div class="alert alert-warning alert-dismissible fade show" role="alert">
        <strong>'.$num.' </strong> Reader(s) found!
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>';
      }
      else{
    ...
    ?>
<div class="newbox">
    <form class="d-flex" action="readersearch.php" method="post">
      <input class="form-control me-2" id="readnamesearch" name="readnamesearch" type="search" placeholder="Search a reader by name" aria-label="Search" required>
      <button class="btn btn-success" name="searchbutton" id="searchbutton" type="submit">Search</button>
    </form>
</div>
<button class="btn btn-success badge-pill btn-lg" name="addreadbutton" id="addreadbutton" onClick="location.href='addreader.php' "><b>Add Reader</b></button>
